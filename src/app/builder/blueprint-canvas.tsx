  ...
  useEffect(() => {
    ...
    function onKeyDown(ev: KeyboardEvent) {
      ...
      if (!meta) {
        ...
        if (key === "escape") { cancelDrag(); setCalibration(null); return; }
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: "grid", gridTemplateColumns: mapOnly ? "minmax(0,1fr)" : `${leftWidth}px 6px minmax(360px,1fr) 6px ${rightWidth}px` }}>
      ...
      <div style={{ minWidth: 0, display: "flex", flexDirection: "column", background: "var(--color-bg)", position: "relative" }}>
        ...
        <div
          ref={wrapRef}
          className={`canvas-wrap canvas-mode-${panActive ? "navigate" : effectiveMode}${panning ? " is-panning" : ""}${calibration ? " is-calibrating" : ""}`}
          style={{ flex: 1, minHeight: 0, position: "relative", overflow: "auto" }}
          // Capture phase: while calibrating, a click anywhere on the floor is
          // a measurement point, even over an object.
          onMouseDownCapture={(e) => {
            if (!calibration || e.button !== 0) return;
